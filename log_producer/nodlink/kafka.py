...
from config import event_csv_predict_list, log_list, event_dict
...
def send_log(json_file_path, producer, topic):
    ...
    with open(json_file_path, 'r') as json_file:
        for i, jsonline in enumerate(json_file):
            ...
            log['host_uuid'] = '00000000-0000-0000-0000-000000000001'
            log['event_uuid'] = str(uuid.UUID(bytes=log['event_uuid'].to_bytes(16, byteorder='big')))
...
if __name__ == "__main__":
    ...
    for predict_data_path, json_file_path in zip(event_csv_predict_list, log_list):
        ...
        send_log(json_file_path, producer, f"nodlink-{count}")
        print(f"jsonCount: {json_count} \n jsonSentCount: {json_sent_count}")
        print("continue...\n")
        producer.flush()
        json_count = 0
